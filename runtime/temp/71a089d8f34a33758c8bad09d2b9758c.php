<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:83:"D:\wamp3\wamp64\www\recruit\public/../application/admin\view\course\courselist.html";i:1517555568;}*/ ?>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
		...
		layui.use([ 'table', 'layer','jquery','form' ], function() {
			...
			var tableIns = table.render({
				        ...
				        cols:[[
                         ...
                         {field:'is_top',title:"置顶",templet: '#top'},
	...
	//对课程进行操作
		table.on('tool(table)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
			 ...
			 if(layEvent === 'del'){ //删除
              ...
              }else if(layEvent === 'top2on'){
                  _ajax("<?php echo url('topChange'); ?>",{courseid:data.courseid,is_top:1},dtd)
                  dtd.done(function(){
                      $(tr).find("button.topon").get(0).outerHTML='<button class="layui-btn layui-btn-warm layui-btn-xs topoff" lay-event="top2off">撤销</button>'
                      obj.update({
                          is_top:1
                      });
                  })

               }else if(layEvent === 'top2off'){
                   _ajax("<?php echo url('topChange'); ?>",{courseid:data.courseid,is_top:0},dtd)
                   dtd.done(function(){
                       $(tr).find("button.topoff").get(0).outerHTML=' <button class="layui-btn layui-btn-xs topon" lay-event="top2on">激活</button>'
                       obj.update({
                           is_top:0
                       });
                   })
...
<script type="text/html" id="top">
    {{#  if(d.is_top==1){ }}
    <button class="layui-btn layui-btn-warm layui-btn-xs topoff" lay-event="top2off">撤销</button >
    {{#  } else { }}
        <button class="layui-btn layui-btn-xs topon" lay-event="top2on">激活</button>
    {{#  } }}
</script>
<script type="text/html" id="bar">
  <button class="layui-btn layui-btn-xs" lay-event="edit">编辑</button>
  <button class="layui-btn layui-btn-xs" lay-event="user">学员管理</button>
  {{#  if(d.status== 1){ }}
