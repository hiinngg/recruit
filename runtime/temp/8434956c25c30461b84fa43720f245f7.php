<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:77:"D:\wamp6\wamp64\www\recruit\public/../application/admin\view\job\jobcate.html";i:1517125660;}*/ ?>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
...
<!-- 		<button class="layui-btn layui-btn-danger">
		...
		<button class="layui-btn layui-btn-normal addcate" >
			<i class="layui-icon">&#xe654;</i>新增职业类目
		</button> 
	...
	var tranStatus={
		...
		var jocateTable = table.render({
			        elem:"#table",	
			       
			        url: "<?php echo url('job/jobcate'); ?>",
			        cols:[[
			         {checkbox: true},
			         {field: 'cateid', title: '编号',type:"numbers" },
			         {field: 'name', title: '名称' },
	            	 ...
	            	 layer.prompt({
	            		  ...
	            		  $.ajax({
	            			  ...
	            			  success:function(data){
	            	             if(data==1){
	            	            	 if(typeof(jocateTable)!="undefined"){
	            	            		 jocateTable.reload() 
	            	            	 }else{
	            	            		 history.go(0)
	            	            	 }
				...
				table.on('tool(table)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
					  ...
					  if(layEvent === 'detail'){ //查看
					    ...
					    layer.confirm('确定删除该分类么', function(index){
					    	  _ajax("<?php echo url('cateDel'); ?>",{cateid:data.cateid},dtd)
							  dtd.done(function(){
								  obj.del(); 
								  layer.close(index);
							  })
						  ...
						  layer.prompt({
		            		  ...
		            		  title:"请输入新的分类名称"
		            			  ...
		            			  $.ajax({
			            			  ...
			            			  data:{name:value,cateid:data.cateid},
			            			  beforeSend:function(){
			            				  layer.load(2)
			            			  },
			            			  success:function(data){
