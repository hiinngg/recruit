		  ...
		  //初始化layer.tree
		   var tree = layui.tree({
			...
			window.onload=function(){
	  			  ...
	  			  //添加操作的图标
	  			  $("ul#demo").find("a").after("<i  class='layui-icon select hide add' )'>&#xe608;</i> <i    class='layui-icon del select hide'>&#xe640;</i> ")	  
		...
		layer.prompt({title: '输入分类名称，并确认', formType:0}, function(text, index){
			 ...
			 $.ajax({
				 ...
				 success:function(data){
					 layer.closeAll();
					 if(data.code==1){
						 $("ul#demo").append("<li  pid='0'   id="+(data.cateid)+"><a   ><cite>"+text+"</cite> </a><i  class='layui-icon select hide add' )'>&#xe608;</i>  <i    class='layui-icon del select hide'>&#xe640;</i></li>");
					 }else{
						 layer.msg(data)
					 }
				 },
			 ...
			 success:function(data){
				 ...
				 if(data.code==1){
					 if(that.children("ul").length == 0){
						 ...
						 that.append("<ul class='layui-show'><li  pid="+pid+"   id="+(data.cateid)+"><a    ><cite>"+text+"</cite> </a><i  class='layui-icon select hide add' )'>&#xe608;</i>  <i    class='layui-icon del select hide'>&#xe640;</i></li></ul>")
					 }else{
						that.children("ul").append("<li pid="+pid+"    id="+(data.cateid)+"><a  ><cite>"+text+"</cite> </a><i  class='layui-icon select hide add' )'>&#xe608;</i>  <i class='layui-icon del select hide'>&#xe640;</i></li>");
					 }
				 }else{
					 layer.msg(data)
				 }
